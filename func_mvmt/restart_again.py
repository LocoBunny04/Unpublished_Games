...
wn = trtl.Screen()
#si = tk.Tk()
si = trtl.Turtle();caller = trtl.Turtle();st = trtl.Turtle();rSt = trtl.Turtle();user = trtl.Turtle();point = trtl.Turtle(); atmpt = trtl.Turtle()
...
def rStPress(x, y):
    ...
    user.shape(user_pic)
    wn_msg = ''
    wn_msg = trtl.textinput('Leaderboard','Enter your Name for Leaderboard')
    wn.delay(500)
    # while rP == True:
    #     break
    # if rP == False: 
...
def rst_trace(rP):
    global lead, count, attempt, rP_count
    rP = True
    rP_count += 1
    while rP == True:
        if rP_count == 1:
